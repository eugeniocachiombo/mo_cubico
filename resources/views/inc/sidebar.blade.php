<!-- Sidebar Start -->
<div class="sidebar {{auth()->user()->getDarkMode ? '' : 'bg-white' }} pe-4 pb-3">
    <nav class="navbar {{auth()->user()->getDarkMode ? 'bg-secondary' : 'bg-white' }} navbar-dark">
        ...
        <div class="d-flex align-items-center ms-4 mb-4">
            ...
            <div class="ms-3 ">
                <h6 class="mb-0 {{auth()->user()->getDarkMode ? '' : 'text-dark' }}">{{ ucwords(auth()->user()->first_name) }}
                </h6>
        ...
        <div class="navbar-nav w-100">
            <a href="{{ route('home.dashboard') }}"
                class="nav-item nav-link {{ \Illuminate\Support\Facades\Route::currentRouteName() == 'home.dashboard' ? auth()->user()->getDarkMode ? 'active' : 'active_whitemode'  : '' }}"><i
                    class="fa fa-tachometer-alt me-2"></i>Painel</a>
            <a href="{{ route('user.profile') }}"
                class="nav-item nav-link {{ \Illuminate\Support\Facades\Route::currentRouteName() == 'user.profile' ? auth()->user()->getDarkMode ? 'active' : 'active_whitemode'  : '' }}"><i
                    class="fa fa-user me-2"></i>Perfil</a>
            <a href="{{ route('home-rent') }}"
                class="nav-item nav-link {{ \Illuminate\Support\Facades\Route::currentRouteName() == 'home-rent' ? auth()->user()->getDarkMode ? 'active' : 'active_whitemode'  : '' }}""><i
                    class="fa fa-home me-2"></i>Imóveis</a>
