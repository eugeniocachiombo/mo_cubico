<div class="position-relative ">
    ...
    <button type="button" class="btn btn-link p-0 m-0" onclick="document.getElementById('photoUploadInput').click();">
        <i wire:loading.remove wire:target="photo" class="fa fa-camera {{auth()->user()->getDarkMode ? 'text-white' : 'text-dark' }}" style="font-size: 20px;"></i>
    </button>

    <div wire:loading wire:target="photo" class="text-muted small">
        <i class="fa fa-spinner fa-spin {{auth()->user()->getDarkMode ? '' : 'text-dark' }}"></i> 
    </div>
